...
function ScrollTabfaq() {
  ...
  const renderFaqSection = (id) => (
    <div id={id} className={`row-what-we-do-inner-3 ${activeId1 === id ? 'active' : ''}`}>
      <div className="signlefaqlink">
        {['Ultra-Slim Sliding Windows', 'Bi-Fold Windows', 'Casement Windows', 'Curtain Wall Windows'].map((item, idx) => (
      ...
      <div className="signlefaqpage contact_formfaq">
        {faqs.map((faq, index) => (
          <div className={`card ${activeIndex === index ? 'active' : ''}`} key={index}>
            <div className="card-header" onClick={() => toggleAccordion(index)} style={{ cursor: 'pointer' }}>
              <span className="card-link">{faq.title}</span>
            </div>
            {activeIndex === index && <div className="card-body">{faq.content}</div>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <section className="what-we-do what-we-do-faqs" style={{ backgroundImage: `url('/images/white_bg.jpg')` }}>
      <div className="container-fluid">
        <div className="row row_justify">
          <div className="col-lg-12 col-md-12">
            <div className="main-heading">
              <h2 className="heading">Frequently Asked Questions</h2>
              <p className="para">Welcome to Schon — where your quest for luxury and practicality in home living finds its answer.</p>
            </div>
          </div>
